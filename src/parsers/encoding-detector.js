...
class EncodingDetector {
    ...
    static detectBOM(buffer) {
        for (const pattern of types_1.ENCODING_PATTERNS) {
    ...
    static calculateUTF8Confidence(stats) {
        if (stats.nullBytes > 0)
            return 0;
        if (stats.controlChars > stats.total * 0.1)
            return 0.3;
        const asciiRatio = stats.asciiChars / stats.total;
        const nonAsciiRatio = stats.nonAsciiChars / stats.total;
        if (asciiRatio > 0.95)
            return 0.95;
        if (asciiRatio > 0.8 && nonAsciiRatio < 0.2)
            return 0.9;
        if (stats.highBytes === 0 && nonAsciiRatio < 0.3)
            return 0.85;
        return 0.7;
    }
    static detectUTF16(_buffer, stats) {
