...
exports.CSVDetector = void 0;
const types_1 = require("./types");
const encoding_detector_1 = require("./encoding-detector");
class CSVDetector {
    /**
    ...
    static detect(buffer) {
        ...
        const encodingResult = encoding_detector_1.EncodingDetector.detect(buffer);
        // Skip BOM if present
    ...
    static detectLineEnding(sample) {
    ...
    static detectDelimiter(lines) {
        ...
        const analyses = [];
        for (const delimiter of types_1.COMMON_DELIMITERS) {
    ...
    static analyzeDelimiter(lines, delimiter) {
        if (lines.length < 2) {
        ...
        const variance = fieldCounts.reduce((sum, count) => sum + Math.pow(count - meanFields, 2), 0) /
            fieldCounts.length;
    ...
    static countFields(line, delimiter) {
        // Simple field counting - doesn't handle quotes yet
        return line.split(delimiter).length;
    }
    static detectQuoteCharacter(lines, delimiter) {
        let bestQuote = '"';
        let bestConfidence = 0;
        for (const quote of types_1.COMMON_QUOTES) {
    ...
    static analyzeQuoteCharacter(lines, delimiter, quote) {
        let quotedFields = 0;
        let totalFields = 0;
    ...
    static detectHeader(lines, delimiter, quote) {
        if (lines.length < 2) {
    ...
    static looksLikeHeader(headerField, dataField) {
        ...
        if (!/[a-zA-Z]/.test(headerField))
            return false;
        // Headers are usually shorter and more descriptive
        if (headerField.length > 50)
            return false;
...
exports.CSVDetector = CSVDetector;
//# sourceMappingURL=csv-detector.js.map