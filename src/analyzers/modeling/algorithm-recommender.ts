...
export class AlgorithmRecommender {
  ...
  async recommendAlgorithms(
    ...
    return recommendations
      ...
      .slice(0, 5); // Top 5 recommendations per task
  }
  ...
  private generateClusteringRecommendations(
    ...
    if (dataSize < 1000) { // Computational limitation
      recommendations.push({
  ...
  private calculateLinearRegressionSuitability(task: ModelingTask, section3Result: Section3Result): number {
    let score = 75; // Base score
    
    // Adjust based on complexity preference
    if (this.config.complexityPreference === 'simple') score += 15;
    if (this.config.interpretabilityRequirement === 'high') score += 10;
    
    return Math.min(100, score);
  }

  private calculateTreeSuitability(task: ModelingTask, section3Result: Section3Result, taskContext: string): number {
    let score = 80; // Base score
    
    // Trees are great for interpretability
    if (this.config.interpretabilityRequirement === 'high') score += 15;
    if (this.config.complexityPreference === 'moderate') score += 5;
    
    return Math.min(100, score);
  }

  private calculateEnsembleSuitability(task: ModelingTask, section3Result: Section3Result, dataSize: number): number {
    let score = 85; // Base score
    
    // Ensembles need sufficient data
    if (dataSize < 100) score -= 20;
    if (dataSize > 1000) score += 10;
    
    // Complexity considerations
    if (this.config.complexityPreference === 'simple') score -= 10;
    if (this.config.interpretabilityRequirement === 'high') score -= 15;
    
    return Math.min(100, Math.max(0, score));
  }

  private calculateLogisticRegressionSuitability(task: ModelingTask, section3Result: Section3Result): number {
    let score = 80; // Base score
  ...
  private calculateRegularizedRegressionSuitability(task: ModelingTask, section3Result: Section3Result, featureCount: number): number {
    let score = 70; // Base score
    
    // More features = more beneficial
    if (featureCount > 10) score += 15;
    if (featureCount > 20) score += 10;
    
    return Math.min(100, score);
  }

  private calculateKMeansSuitability(task: ModelingTask, section3Result: Section3Result, dataSize: number): number {
    let score = 80;
  ...
  private calculateHierarchicalSuitability(task: ModelingTask, section3Result: Section3Result, dataSize: number): number {
    let score = 75;
    
    // Penalize for large datasets
    if (dataSize > 500) score -= 20;
    if (this.config.interpretabilityRequirement === 'high') score += 15;
  ...
  private getTreeHyperparameters(taskType: string): HyperparameterGuide[] {
    return [
      {
  ...
  private getRandomForestHyperparameters(): HyperparameterGuide[] {
    return [
      {
