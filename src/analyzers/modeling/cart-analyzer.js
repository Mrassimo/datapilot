...
exports.CARTAnalyzer = void 0;
const logger_1 = require("../../utils/logger");
class CARTAnalyzer {
    /**
     * Generate comprehensive CART analysis
     */
    async generateCARTAnalysis(tasks, treeAlgorithms) {
        logger_1.logger.info('Generating CART (Decision Tree) analysis');
     ...
     */
    generateMethodologyDescription(isRegression) {
...
- Instability: Sensitive to data perturbations`;
    ...
    getRegressionMathematicalFoundation() {
...
**Prediction:** For a leaf node, prediction = mean of target values in that leaf`;
    ...
    getClassificationMathematicalFoundation() {
...
**Probability Estimation:** P(class i) = proportion of class i samples in leaf`;
     ...
     */
    determineSplittingCriterion(isRegression) {
        if (isRegression) {
     ...
     */
    generatePruningStrategy(isRegression) {
        return {
...
- Monitor both training and validation performance during pruning`,
     ...
     */
    generateTreeInterpretation(task) {
     ...
     */
    generateExampleDecisionPaths(task) {
        if (!task)
            return [];
        const paths = [];
    ...
    generateBusinessRules(task) {
        ...
        rules.push(`IF-THEN Rule Translation: Decision trees naturally translate to business rules`, `Each path from root to leaf represents a complete business rule`, `Rules are mutually exclusive and collectively exhaustive`, `Example structure: IF (condition1 AND condition2) THEN predict ${targetVar} = value`);
        if (taskType === 'regression') {
            rules.push(`For regression: Each leaf provides a numerical prediction (mean of training samples in leaf)`, `Rules can be used for segmentation: "High ${targetVar} segment", "Low ${targetVar} segment"`, `Confidence intervals can be calculated using standard deviation in each leaf`);
        }
        else {
            rules.push(`For classification: Each leaf provides class prediction and probability estimates`, `Rules can include confidence levels based on class purity in leaves`, `Probability estimates help with decision thresholds and uncertainty quantification`);
        }
        return rules;
    }
    /**
...
- Highlight misclassified instances`;
     ...
     */
    generateFeatureImportanceGuidance(task) {
        if (!task)
            return [];
        return task.inputFeatures.slice(0, 5).map((feature, index) => ({
            ...
            rank: index + 1,
            confidenceInterval: [0.8 - index * 0.15 - 0.1, 0.8 - index * 0.15 + 0.1],
     ...
     */
    generateVisualizationRecommendations(isRegression) {
        const common = [
...
exports.CARTAnalyzer = CARTAnalyzer;
//# sourceMappingURL=cart-analyzer.js.map