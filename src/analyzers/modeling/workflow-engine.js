...
class WorkflowEngine {
     ...
     */
    async generateWorkflow(tasks, algorithms, section1Result, section5Result) {
        logger_1.logger.info('Generating modeling workflow guidance');
        ...
        return {
            ...
            dataSplittingStrategy: this.generateDataSplittingStrategy(primaryTaskType, dataSize, hasTemporalData),
            crossValidationApproach: this.generateCrossValidationApproach(primaryTaskType, dataSize, hasTemporalData),
     ...
     */
    generateWorkflowSteps(tasks, algorithms, dataSize, hasTemporalData) {
        const steps = [];
        ...
        if (tasks.some((task) => task.taskType === 'regression')) {
            steps.push({
                ...
                description: 'Perform detailed residual analysis and assumption checking for regression models',
     ...
     */
    generateBestPractices(primaryTaskType, dataSize) {
        const practices = [];
     ...
     */
    generateDataSplittingStrategy(primaryTaskType, dataSize, hasTemporalData) {
        if (hasTemporalData) {
            return {
                strategy: 'temporal',
                trainPercent: 70,
                ...
                testPercent: 15,
                reasoning: 'Temporal data requires chronological splits to simulate real-world prediction scenarios',
                implementation: 'Split data chronologically: oldest 70% for training, next 15% for validation, newest 15% for testing',
                considerations: [
        ...
        if (primaryTaskType.includes('classification')) {
            return {
                ...
                reasoning: 'Stratified sampling ensures representative class distribution across all splits',
                implementation: 'Use stratified random sampling to maintain class proportions in train/validation/test sets',
                considerations: [
     ...
     */
    generateCrossValidationApproach(primaryTaskType, dataSize, hasTemporalData) {
        if (hasTemporalData) {
            return {
                method: 'time_series_split',
                folds: Math.min(5, Math.floor(dataSize / 100)),
                reasoning: 'Time series cross-validation respects temporal ordering and simulates realistic forecasting scenarios',
                implementation: 'Use TimeSeriesSplit from scikit-learn with expanding or sliding window approach',
        ...
        if (primaryTaskType.includes('classification')) {
            return {
                ...
                reasoning: 'Stratified cross-validation maintains class distribution in each fold, providing stable performance estimates',
                implementation: 'Use StratifiedKFold from scikit-learn to maintain class proportions across folds',
     ...
     */
    generateHyperparameterTuningStrategy(algorithms, dataSize) {
        const complexAlgorithms = algorithms.filter((alg) => alg.complexity === 'moderate' || alg.complexity === 'complex');
        const method = dataSize > 10000 && complexAlgorithms.length > 0 ? 'random_search' : 'grid_search';
        return {
            method,
            ...
            earlyStoppingCriteria: method === 'random_search' ? 'No improvement for 50 iterations' : undefined,
        };
    }
    // Helper methods
    detectTemporalData(tasks) {
        return tasks.some((task) => task.taskType === 'time_series_forecasting' ||
            task.inputFeatures.some((feature) => feature.toLowerCase().includes('date') || feature.toLowerCase().includes('time')));
    }
    identifyPrimaryTaskType(tasks) {
        if (tasks.length === 0)
            return 'unknown';
    ...
    generateSearchSpaces(algorithms) {
    ...
    selectOptimizationMetric(algorithms) {
        // Simplified logic - would analyze task types
        const hasClassification = algorithms.some((alg) => alg.evaluationMetrics.includes('Accuracy') || alg.evaluationMetrics.includes('F1-Score'));
        return hasClassification ? 'f1_macro' : 'neg_mean_squared_error';
    }
    generateEvaluationFramework(tasks, algorithms) {
    ...
    generateInterpretationGuidance(algorithms) {
        // Simplified implementation
        return {
