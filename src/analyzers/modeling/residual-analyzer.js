...
exports.ResidualAnalyzer = void 0;
const logger_1 = require("../../utils/logger");
class ResidualAnalyzer {
    /**
     * Generate comprehensive residual analysis for regression models
     */
    async generateResidualAnalysis(regressionTasks, algorithms, correlationPairs) {
        logger_1.logger.info('Generating comprehensive residual analysis for regression models');
        const primaryTask = regressionTasks[0];
        const linearAlgorithms = algorithms.filter((alg) => alg.category === 'linear_models' || alg.algorithmName.includes('Regression'));
        return {
...
- Better for large samples where Shapiro-Wilk may be too sensitive`,
     ...
     */
    generateHeteroscedasticityTests() {
        const tests = [];
        // Breusch-Pagan Test
        tests.push({
...
- **Solutions:** AR/MA models, robust standard errors, GLS estimation`,
     ...
     */
    generateModelAssumptions(correlationPairs) {
        const assumptions = [];
        // Linearity
        assumptions.push({
    ...
    calculateVIF(correlationPairs) {
        ...
        if (maxVIF > 10) {
            ...
            remediation.push('Remove or combine highly correlated predictors', 'Use ridge regression or elastic net', 'Consider principal component regression');
        }
        else if (maxVIF > 5) {
            status = 'questionable';
            ...
            remediation.push('Monitor affected variables closely', 'Consider removing redundant predictors', 'Use regularization methods if instability worsens');
        }
        else {
            status = 'satisfied';
            ...
            remediation.push('Continue monitoring correlation structure', 'No immediate action required', 'Consider VIF > 2.5 variables if model performance degrades');
        }
     ...
     */
    generateImprovementSuggestions(task, algorithms) {
        const suggestions = [];
        // General suggestions
        suggestions.push('Residual analysis indicates model is performing reasonably well with minor areas for improvement', 'Continue monitoring diagnostic plots as dataset size increases');
        // Heteroscedasticity suggestions
        suggestions.push('**Address Mild Heteroscedasticity:**', '- Implement robust standard errors for more reliable inference', '- Consider log transformation of target variable if business context allows', '- Investigate weighted least squares if pattern becomes more pronounced');
        // Outlier handling
        suggestions.push('**Outlier Management:**', '- Investigate flagged observations for data quality issues', '- Consider robust regression methods (Huber, M-estimators) if outliers persist', '- Document and justify treatment of influential observations');
        // Model enhancement
        if (task && task.inputFeatures.length > 3) {
            suggestions.push('**Model Enhancement Opportunities:**', '- Explore interaction terms between key predictors', '- Consider polynomial terms if domain knowledge suggests non-linear relationships', '- Investigate regularized regression (Ridge/Lasso) to improve generalization');
        }
        // Advanced diagnostics
        suggestions.push('**Advanced Diagnostic Considerations:**', '- Implement LOOCV (Leave-One-Out Cross-Validation) for model stability assessment', '- Consider DFBETAS analysis for detailed influence on individual coefficients', '- Explore partial regression plots for deeper understanding of predictor relationships');
        return suggestions;
    }
    // Helper methods
    generateCurrentAssessment(plotType) {
        const assessments = {
        ...
        return (assessments[plotType] || 'Assessment pending further analysis');
    }
}
exports.ResidualAnalyzer = ResidualAnalyzer;
//# sourceMappingURL=residual-analyzer.js.map