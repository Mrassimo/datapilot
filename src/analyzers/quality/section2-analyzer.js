...
exports.Section2Analyzer = void 0;
const completeness_analyzer_1 = require("./completeness-analyzer");
const uniqueness_analyzer_1 = require("./uniqueness-analyzer");
const validity_analyzer_1 = require("./validity-analyzer");
const business_rule_engine_1 = require("./business-rule-engine");
const pattern_validation_engine_1 = require("./pattern-validation-engine");
const config_1 = require("../../core/config");
class Section2Analyzer {
    data;
    headers;
    columnTypes;
    rowCount;
    ...
    entityColumnCache = new Set();
    dateColumnCache = new Set();
    numericColumnCache = new Set();
    constructor(input) {
    ...
    isEntityIdentifierColumn(headerLower) {
    ...
    async analyzeCompleteness() {
        const analyzer = new completeness_analyzer_1.CompletenessAnalyzer({
            data: this.data,
    ...
    async analyzeUniqueness() {
        const analyzer = new uniqueness_analyzer_1.UniquenessAnalyzer({
            data: this.data,
            headers: this.headers,
            columnTypes: this.columnTypes,
            rowCount: this.rowCount,
    ...
    async analyzeValidity() {
        const analyzer = new validity_analyzer_1.ValidityAnalyzer({
            data: this.data,
            headers: this.headers,
            columnTypes: this.columnTypes,
            rowCount: this.rowCount,
    ...
    async analyzeBusinessRulesAndPatterns() {
        ...
        const businessRuleEngine = new business_rule_engine_1.BusinessRuleEngine(this.data, this.headers, this.columnTypes, businessRuleConfig);
        ...
        const patternEngine = new pattern_validation_engine_1.PatternValidationEngine(this.data, this.headers, patternConfig);
    ...
    calculateAccuracyScore(businessRuleResults, patternResults) {
        ...
        const criticalViolations = businessRuleResults.criticalViolations +
            (patternResults.patternValidations?.filter((p) => p.severity === 'critical').length ||
                0);
    ...
    calculateConsistencyScore(businessRuleResults, patternResults) {
     ...
     */
    calculateEnhancedConsistencyScore(businessRuleResults, patternResults, statisticalInsights) {
    ...
    extractStatisticalInsights() {
        ...
        const insights = { hasStatisticalTests: true };
        try {
            ...
            if (correlationPairs.length > 0) {
                ...
                if (veryHighCorrelations.length > 0) {
                    ...
                    veryHighCorrelations.forEach((pair) => {
                        if (!affectedVariables.includes(pair.variable1))
                            affectedVariables.push(pair.variable1);
                        if (!affectedVariables.includes(pair.variable2))
                            affectedVariables.push(pair.variable2);
                    });
                }
    ...
    createPlaceholderReasonableness() {
        return {
    ...
    createPlaceholderPrecision() {
        ...
        let details = 'Precision analysis based on numeric scale, temporal granularity, and categorical specificity';
        // Deduct points for precision inconsistencies
        const precisionIssues = numericPrecision.filter((p) => p.inconsistentPrecision).length +
            temporalGranularity.filter((t) => t.mixedGranularity).length +
            categoricalSpecificity.filter((c) => c.lowSpecificity).length;
    ...
    createPlaceholderRepresentational() {
        return {
            unitStandardization: [],
            codeStandardization: [],
            textFormatting: [],
    ...
    createDataQualityCockpit(scores) {
    ...
    identifyWeaknesses(dimensionScores) {
        const weaknesses = [];
        const sortedDimensions = Object.entries(dimensionScores).sort(([, a], [, b]) => a.score - b.score);
    ...
    estimateEffortForDimension(_dimension, score) {
    ...
    estimateTechnicalDebt(dimensionScores, weaknesses) {
        ...
        let complexityLevel;
        if (totalEffortHours > 20)
            complexityLevel = 'High';
        else if (totalEffortHours > 8)
    ...
    countAutomatedFixableIssues(dimensionScores) {
     ...
     */
    performExternalReferenceValidation() {
        ...
        if (externalRefs.countryCodesList) {
            const countryColumns = this.headers
                ...
                .filter(({ header }) => /country|nation|ctry|cntry/i.test(header) ||
                header.toLowerCase().includes('country_code') ||
                header.toLowerCase() === 'cc');
            countryColumns.forEach(({ header, index }) => {
                const violations = this.validateAgainstReferenceList(index, externalRefs.countryCodesList, 'Country Code Standard');
        ...
        if (externalRefs.currencyCodesList) {
            const currencyColumns = this.headers
                .map((header, index) => ({ header, index }))
                .filter(({ header }) => /currency|curr|money/i.test(header) ||
                header.toLowerCase().includes('currency_code') ||
                header.toLowerCase() === 'ccy');
            currencyColumns.forEach(({ header, index }) => {
                const violations = this.validateAgainstReferenceList(index, externalRefs.currencyCodesList, 'Currency Code Standard');
                if (violations.violationsFound > 0) {
                    conformityChecks.push({
                        columnName: header,
        ...
        if (externalRefs.productMasterList) {
            ...
            productColumns.forEach(({ header, index }) => {
                const violations = this.validateAgainstReferenceList(index, externalRefs.productMasterList, 'Product Master List');
                if (violations.violationsFound > 0) {
                    conformityChecks.push({
                        columnName: header,
     ...
     */
    validateAgainstReferenceList(columnIndex, referenceList, standardName) {
        const configManager = (0, config_1.getConfig)();
        const qualityConfig = configManager.getQualityConfig();
    ...
    getEntityColumnsFromCache() {
        ...
        this.entityColumnCache.forEach((index) => {
            ...
            if (lowerHeader.includes('customer') ||
    ...
    analyzeDataFreshness(dateColumns) {
        if (dateColumns.length === 0) {
            return {
    ...
    analyzeTemporalGranularity() {
        const results = [];
        const dateColumns = this.findDateColumns();
        for (const dateCol of dateColumns) {
            const granularityLevels = new Set();
            let sampleCount = 0;
    ...
    analyzeCategoricalSpecificity() {
        ...
        for (let colIndex = 0; colIndex < this.headers.length; colIndex++) {
            ...
            if (sampleCount > 0) {
                ...
                const lowSpecificity = avgLength < 3 || // Very short values
...
exports.Section2Analyzer = Section2Analyzer;
//# sourceMappingURL=section2-analyzer.js.map