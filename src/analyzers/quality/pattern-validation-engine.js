...
exports.PatternValidationEngine = void 0;
class PatternValidationEngine {
    ...
    patterns = [];
    violations = [];
    constructor(data, headers, config = {}) {
        this.data = data;
    ...
    validatePatterns() {
        ...
        for (let colIndex = 0; colIndex < this.headers.length; colIndex++) {
            ...
            const applicablePatterns = this.patterns.filter((p) => p.enabled && p.columnPattern.test(columnName));
     ...
     */
    addUnitStandardizationAnalysis() {
        const unitAnalysis = [];
        for (let colIndex = 0; colIndex < this.headers.length; colIndex++) {
            const columnName = this.headers[colIndex];
            // Weight/mass units
    ...
    analyzeFormatConsistency() {
        ...
        for (let colIndex = 0; colIndex < this.headers.length; colIndex++) {
            ...
            if (/(date|created|updated|birth|expir)/i.test(columnName)) {
                ...
                if (dateFormats)
                    formatAnalysis.push(dateFormats);
            }
            ...
            if (/(phone|tel|mobile|cell)/i.test(columnName)) {
                ...
                if (phoneFormats)
                    formatAnalysis.push(phoneFormats);
            }
            ...
            if (/(is|has|can|should|enabled|active|valid)/i.test(columnName)) {
                ...
                if (booleanFormats)
                    formatAnalysis.push(booleanFormats);
            }
            ...
            if (/(name|title|city|company|description)/i.test(columnName)) {
                ...
                if (casingConsistency)
                    formatAnalysis.push(casingConsistency);
            }
        }
        return formatAnalysis;
    }
    analyzeColumnFormatConsistency(colIndex, columnName, formats) {
        ...
        return {
            ...
            consistency: {
                ...
                inconsistencyPercentage: (((totalValues - dominantFormat.count) / totalValues) *
                    100).toFixed(1),
            },
            score: {
    ...
    analyzeCasingConsistency(colIndex, columnName) {
        const casingPatterns = new Map();
        const examples = new Map();
        let totalValues = 0;
        for (let rowIndex = 0; rowIndex < this.data.length; rowIndex++) {
            const value = this.data[rowIndex]?.[colIndex];
        ...
        return {
            ...
            consistency: {
                ...
                inconsistencyPercentage: (((totalValues - dominantCasing.count) / totalValues) *
                    100).toFixed(1),
            },
            score: {
    ...
    getPatternSummary() {
        ...
        const columnViolations = new Map();
        for (const violation of this.violations) {
            const pattern = this.patterns.find((p) => p.id === violation.patternId);
            if (pattern) {
                violationsBySeverity[pattern.severity]++;
            }
            columnViolations.set(violation.columnName, (columnViolations.get(violation.columnName) || 0) + 1);
        }
...
exports.PatternValidationEngine = PatternValidationEngine;
//# sourceMappingURL=pattern-validation-engine.js.map