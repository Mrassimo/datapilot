 ...
 */
export function fccdf(f: number, df1: number, df2: number): number {
  if (f <= 0) return 1;
 ...
 */
export function chisqccdf(x: number, df: number): number {
  if (x <= 0) return 1;
 ...
 */
export function checkNumericalStability(value: number, context: string): number {
  if (!isFinite(value)) {
