...
import { fccdf, chisqccdf, validateTestInputs, checkNumericalStability } from './distributions';
...
export interface GroupData {
  ...
  mean: number;
  variance: number;
  values?: number[]; // Raw values for non-parametric tests
}
 ...
 */
export function anovaFTest(groups: GroupData[]): StatisticalTestResult {
 ...
 */
export function kruskalWallisTest(groups: GroupData[]): StatisticalTestResult {
  validateTestInputs(groups);
  ...
  const df = k - 1;
  const pValue = chisqccdf(hStatistic, df);
 ...
 */
function assignRanksWithTies(values: number[]): number[] {
 ...
 * ANOVA assumptions
 */
function getAnovaAssumptions(): string[] {
  return [
 ...
 */
function getKruskalWallisAssumptions(): string[] {
  return [
    'Independence: Observations within and between groups are independent',
 ...
 */
export function mannWhitneyUTest(group1: GroupData, group2: GroupData): StatisticalTestResult {
  if (!group1 || !group2) {
 ...
 */
export function andersonDarlingTest(values: number[]): StatisticalTestResult {
...
function mannWhitneyApproximation(group1: GroupData, group2: GroupData): StatisticalTestResult {
...
function calculateMannWhitneyTieCorrection(values: number[]): number {
  const tieGroups = new Map<number, number>();
  for (const value of values) {
    tieGroups.set(value, (tieGroups.get(value) || 0) + 1);
  }
...
function getWelchsAssumptions(): string[] {
  return [
...
function getMannWhitneyAssumptions(): string[] {
  return [
    'Independence: Observations are independent within and between groups',
...
function getAndersonDarlingAssumptions(): string[] {
  return [
...
function generateAndersonDarlingRecommendations(pValue: number, adStat: number): string[] {
  const recommendations: string[] = [];
