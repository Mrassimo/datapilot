...
export class AestheticOptimizer {
  ...
  private static generateOptimizedColorSystem(
    ...
    const accessibilityProfile = this.generateColorAccessibilityProfile(
      primaryPalette,
  ...
  private static expandPaletteUsingHarmony(
    ...
    switch (harmonyType) {
      case 'analogous':
        palette.push(
          this.createColorDefinitionFromHSL(
            { ...baseColor.hsl, hue: (baseHue + 30) % 360 },
            'analogous_1',
          ),
          this.createColorDefinitionFromHSL(
            { ...baseColor.hsl, hue: (baseHue - 30 + 360) % 360 },
            'analogous_2',
          ),
        );
        break;
      case 'complementary':
        palette.push(
          this.createColorDefinitionFromHSL(
            { ...baseColor.hsl, hue: (baseHue + 180) % 360 },
            'complementary',
          ),
        );
        break;
      case 'triadic':
        palette.push(
          this.createColorDefinitionFromHSL(
            { ...baseColor.hsl, hue: (baseHue + 120) % 360 },
            'triadic_1',
          ),
          this.createColorDefinitionFromHSL(
            { ...baseColor.hsl, hue: (baseHue + 240) % 360 },
            'triadic_2',
          ),
        );
        break;
      case 'tetradic':
        palette.push(
          this.createColorDefinitionFromHSL(
            { ...baseColor.hsl, hue: (baseHue + 90) % 360 },
            'tetradic_1',
          ),
          this.createColorDefinitionFromHSL(
            { ...baseColor.hsl, hue: (baseHue + 180) % 360 },
          ...
          this.createColorDefinitionFromHSL(
            { ...baseColor.hsl, hue: (baseHue + 270) % 360 },
            'tetradic_3',
          ),
        );
        break;
    ...
    for (let i = 0; i < hues.length - 2; i++) {
      for (let j = i + 1; j < hues.length - 1; j++) {
        for (let k = j + 1; k < hues.length; k++) {
          ...
          const isTriadic = [diff1, diff2, diff3].every(
  ...
  private static generateAdditionalCategoricalColors(
    existingColors: ColorDefinition[],
    needed: number,
  ): ColorDefinition[] {
    const colors: ColorDefinition[] = [];
  ...
  private static createSpecialPurposeColors(
    ...
    specialColors.set(
      'highlight',
      this.createColorDefinitionFromHSL({ hue: 55, saturation: 95, lightness: 65 }, 'highlight'),
    );

    // Selection color
    specialColors.set(
      'selection',
      this.createColorDefinitionFromHSL({ hue: 210, saturation: 85, lightness: 60 }, 'selection'),
    );
    ...
    specialColors.set(
      'hover',
      this.createColorDefinitionFromHSL(
        { hue: primaryHue, saturation: 70, lightness: 70 },
        'hover',
      ),
    );
  ...
  private static calculateVisualBalance(dataCharacteristics: any, colorSystem: any): any {
    return {
