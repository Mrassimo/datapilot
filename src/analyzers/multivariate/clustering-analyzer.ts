...
import type { KMeansAnalysis, ClusterProfile, ClusterValidationMetrics } from '../eda/types';
...
class ClusterQualityMetrics {
  ...
  static calculateDaviesBouldinIndex(points: ClusterPoint[], centroids: Centroid[]): number {
    ...
    for (const clusterId of clusterIds) {
      ...
      for (const otherClusterId of clusterIds) {
        ...
        const Sj =
          otherClusterPoints.reduce((sum, point) => {
   ...
   */
  static calculateCalinskiHarabaszIndex(points: ClusterPoint[], centroids: Centroid[]): number {
  ...
  static performStabilityAnalysis(
    ...
    const stdStability = Math.sqrt(
      stabilityScores.reduce((sum, score) => sum + Math.pow(score - meanStability, 2), 0) /
...
export class ClusteringAnalyzer {
  ...
  private static performElbowAnalysis(
    ...
    for (let k = 1; k <= maxK; k++) {
      ...
      const silhouetteScore =
        k > 1 ? SilhouetteAnalysis.calculateSilhouetteScore(clustering.points) : 0;
   ...
   * Calculate Within-Cluster Sum of Squares (WCSS)
   */
  private static calculateWCSS(points: ClusterPoint[], centroids: Centroid[]): number {
    let wcss = 0;
   ...
   */
  private static generateClusterDescription(distinctiveFeatures: string[], size: number): string {
  ...
  private static calculateValidationMetrics(points: ClusterPoint[]): ClusterValidationMetrics {
    ...
    const calinskiHarabaszIndex = ClusterQualityMetrics.calculateCalinskiHarabaszIndex(
      points,
      centroids,
    );
  ...
  private static generateInsights(
    ...
    if (validation.silhouetteScore > 0.5) {
      insights.push(
        `Strong clustering structure detected (silhouette score: ${validation.silhouetteScore.toFixed(3)})`,
      );
    } else if (validation.silhouetteScore > 0.25) {
      insights.push(
        `Moderate clustering structure detected (silhouette score: ${validation.silhouetteScore.toFixed(3)})`,
      );
    } else {
      insights.push(
        `Weak clustering structure (silhouette score: ${validation.silhouetteScore.toFixed(3)})`,
      );
    }
    ...
    if (validation.daviesBouldinIndex && validation.daviesBouldinIndex < 1.0) {
      insights.push(
        `Compact, well-separated clusters (Davies-Bouldin: ${validation.daviesBouldinIndex.toFixed(3)})`,
      );
    } else if (validation.daviesBouldinIndex && validation.daviesBouldinIndex > 2.0) {
      insights.push(
        `Overlapping clusters detected (Davies-Bouldin: ${validation.daviesBouldinIndex.toFixed(3)})`,
      );
    }
    ...
    if (validation.calinskiHarabaszIndex && validation.calinskiHarabaszIndex > 10) {
      insights.push(
        `Strong between-cluster separation (Calinski-Harabasz: ${validation.calinskiHarabaszIndex.toFixed(1)})`,
      );
    }

    // Variance explained insight
    insights.push(
      `Clustering explains ${(validation.varianceExplainedRatio * 100).toFixed(1)}% of total variance`,
    );
  ...
  private static generateRecommendations(
    ...
    if (validation.silhouetteScore < 0.25) {
      recommendations.push(
        'Consider feature engineering or different clustering approach due to weak structure',
      );
    }

    if (validation.varianceExplainedRatio < 0.3) {
      recommendations.push(
        'Low variance explained - consider dimensionality reduction before clustering',
      );
    }
    ...
    if (optimalK <= 2) {
      recommendations.push(
        'Dataset may have limited natural clustering - verify with domain knowledge',
      );
    } else if (optimalK >= 7) {
      recommendations.push(
        'Many clusters detected - consider hierarchical clustering for better interpretation',
      );
    }
    ...
    if (coefficient_of_variation > 0.5) {
      recommendations.push(
        'Unbalanced clusters - consider different initialization or clustering algorithm',
      );
    }
    ...
    if (allDistinctiveFeatures.length === 0) {
      recommendations.push(
        'No strong cluster characteristics found - consider feature selection or transformation',
      );
    }
